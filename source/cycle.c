...
void usart_process (BYTE n_port)//вход
{
	...
	switch(port_stat[n_port-1])
	{
		case UCMD_CH:
			...
			{
				port_stat[n_port-1] = UCMD_DWN; //статус дл€ свитч-кейса
				...
				else						{port_time[n_port-1] = eth_wait + TIMER_COEF;	return;}//альтернативна€ установка времени проверки
			}
		...
		case UCMD_DWN://DWN
			if (cfg.sock_rs485[1].mode == TCP_MODE){len_mes = port_udp[n_port].ptr_rx_buf - last_ptr_rx_buf[n_port-1];} /////////////////////////////danger не мен€етс€ проверочный номер 
