...
//len write	
	wr_header(ptr_cnt, KEY_EXCHANGE_INIT, 0);
//eth sock len write
	ssh_eth_sock_upd();
}
...
//define
	...
	static BYTE HSD[] = {0x00,0x00,0x01,0x80};
//read messege
...
//KEX exchange	 SPESIAL TABULATION
	...
	//kex host signature
		...
		//BYTE padding_len = (ptr_cnt + 4) % 8;
		//random_gen(padding_len, &ssh.messege[ptr_cnt]);	
		BYTE pad = 9;//server host  key invalid ?????						
		memset(&ssh.messege[ptr_cnt],0,pad);											ptr_cnt += pad;
//padding
	wr_header(ptr_cnt, DH_KEY_EXCHANGE_REPLY, 0);
	//*
	//new key
	tmp = ptr_cnt;//remember
		ptr_cnt += 6;
		BYTE padding_len = 10;
		memset(&ssh.messege[ptr_cnt],0,padding_len);									ptr_cnt += padding_len;
		
	//wr_header(ptr_cnt - tmp + 4, DH_KEY_NEW_KEY, tmp);
	BYTE tmps = ptr_cnt - tmp;
	ssh_len_w(tmps - 4, &ssh.messege[tmp]);													tmp += 4;
	ssh.messege[tmp] = padding_len;														tmp++;
	ssh.messege[tmp] = DH_KEY_NEW_KEY;
	//*/
	ssh_eth_sock_upd();
}

void wr_header(DWORD ptr_cnt, BYTE mess_code, WORD offset_place)
{
	ssh_len_w(ptr_cnt - 4, &hdr.pack_len);
	hdr.padding_len = SSH_PADDING_LEN;
	hdr.messege_code = mess_code;
	memcpy(&ssh.messege[0 + offset_place],&hdr.pack_len,sizeof(SSH_HEADER));
}
