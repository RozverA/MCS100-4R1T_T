...
void init(BYTE n_port)
{
	...
	port[n_port].sercom->USART.CTRLB.bit.PMODE   =0x00;  // Bit  13    - PMODE: Parity Mode (0: Even parity)
	...
	port[n_port].sercom->USART.CTRLB.bit.SBMODE  =0x00;  // Bit  6     - SBMODE: Stop Bit Mode (0: One stop bit.)
	port[n_port].sercom->USART.CTRLB.bit.CHSIZE  =0x00;  // Bits 2:0   - CHSIZE: Character Size (0: 8 bits)

	port[n_port].sercom->USART.BAUD.bit.BAUD=65536.0f*(1.0f-(8.0*(float)(38400))/(float)(PROC_HERZ));
